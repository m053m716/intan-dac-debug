function [fig,h] = plotWaves(figHandle,axHandle,varargin)
%% PLOTWAVES Plot all the waveforms of channels in a particular folder.
%
%  fig = plotWaves(figHandle,axHandle);
%  fig = plotWaves(figHandle,axHandle,'NAME',value,...)
%
%   --------
%    INPUTS
%   --------
...
%    OUTPUT
%   --------
%     fig        :   Handle to figure with all channels plotted and labeled.
%
% Adapted By: Max Murphy  v1.4 2019-02-02 (from PLOTCHANNELS)
...
if nargin < 2
   fig = figure('Name','Single-Channel Data', ...
      'Units','Normalized', ...
      'Position',[0.05*rand+0.1,0.05*rand+0.1,0.8,0.8]);
   h = axes(fig,'NextPlot','add');
else
   ...
   if isa(axHandle,'matlab.graphics.axis.Axes')
      h = axHandle;
...
if AUTOSAVE
   if isempty(TAG)
      ...
      savefig(gcf,fullfile(block,[name '_' TAG '.fig']));
