...
%% specify fimath for all
F_sum_19_prod_36=fimath('SumMode','KeepMSB','SumWordLength',19,'ProductMode','KeepMSB','ProductWordLength',36,'OverflowAction','wrap');

HPF_coefficient = fi(2*filterCoefficient,1,18,0);
HPF_coefficient.fimath=F_sum_19_prod_36;
...
DAC_input = fi(DAC_input,1,16,0);
HPF_state = fi(HPF_state,1,32,0); 
F_sum_32_prod_36=fimath('SumMode','KeepMSB','SumWordLength',32,'ProductMode','KeepMSB','ProductWordLength',36,'OverflowAction','wrap');
HPF_state.fimath=F_sum_32_prod_36;%% 19 is wrong
...
N = length(DAC_input);
for curr_sample = 1:N
    DAC_input_two_comp(curr_sample) = bitset(DAC_input(curr_sample),16,bitcmp(bitget(DAC_input(curr_sample),16))); 
    HPF_input = fi(DAC_input_two_comp(curr_sample)*4,1,18,0);
    HPF_input.fimath=F_sum_19_prod_36;
    
    % now shift to the left of two positions. 
%     HPF_state_31_14=fi;
%     HPF_state_31_14.fimath=F_sum_19_prod_36;
    HPF_state_31_14 = fi(bitsliceget(HPF_state,32,15),1,18,0); % to me this must be unsigned otherwise when state=0, cmp(0)=1111 interpreted as -1 if yb was signed
    HPF_state_31_14.fimath=F_sum_19_prod_36;
...
%     multiplier_in_before_limit = HPF_input - HPF_state_31_14; %% if they are signed, what about doing this: HPF_input - HPF_state
    
    negative_overflow = getmsb(HPF_input) & bitcmp(getmsb(HPF_state)) & bitcmp(bitget(multiplier_in_before_limit,18)); % multiplier_in_before_limit in verilog is 19bits but it checks the 18th bit
