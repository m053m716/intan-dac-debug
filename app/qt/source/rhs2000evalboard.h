...
public:
    ...
    void setDacWindowStart(int dacChannel, int sample);
    void setDacWindowStop(int dacChannel, int sample);
    void setDetectMode(bool enable);
    bool getDetectMode();
    bool detectMode; // regular (threshold) vs. FSM
    void changeThresholdType(int dacChannel, int type);
    int setMaxWindowStop(int dacChannel, int sample);
    // END UPDATE
    ...
    void setTtlOutMode(QVector<bool> enable);
    bool getTtlOutMode(int channel);
...
//      unused = 0x15,
        ...
        // MM - DISCRIMINATOR - UPDATE - 1/16/2018
        TrigInDacWindow = 0x47,
        TrigInDacDetector = 0x48,
        // END UPDATE
