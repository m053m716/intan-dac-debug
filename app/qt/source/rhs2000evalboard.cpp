...
void Rhs2000EvalBoard::initialize()
{
    ...
    for (stream = 8; stream < 16; stream++) {
        ...
        programStimReg(stream, 0, DacBaseline, 32678);
        programStimReg(stream, 0, DacPositive, 32678 + 3200);
        programStimReg(stream, 0, DacNegative, 32678 - 3200);
    }
...
// Update the Max window length of the discriminator
int Rhs2000EvalBoard::setMaxWindowStop(int dacChannel, int sample)
{
    lock_guard<mutex> lockOk(okMutex);

    if (dacChannel <0 || dacChannel > 7) {
        cerr << "Error in Rhs2000EvalBoard::setMaxWindowStop: dacChannel out of range." << endl;
        return maxWindowStop;
    }

    if (sample < 0 || sample > 65535) {
        cerr << "Error in Rhs2000EvalBoard::setMaxWindowStop: sample out of range." << endl;
        return maxWindowStop;
    }
...
void Rhs2000EvalBoard::setDacWindowStart(int dacChannel, int sample)
{
    lock_guard<mutex> lockOk(okMutex);

    if (dacChannel < 0 || dacChannel > 7) {
        cerr << "Error in Rhs2000EvalBoard::setDacWindowStart: dacChannel out of range." << endl;
        return;
    }

    if (sample < 0 || sample > 65535) {
        cerr << "Error in Rhs2000EvalBoard::setDacWindowStart: sample value out of range." << endl;
        return;
    }

    // Set sample window start.
    dev->SetWireInValue(WireInMultiUse, sample);
    dev->UpdateWireIns();
...
// Set sample index for start of a threshold "window" edge for FSM detector
void Rhs2000EvalBoard::setDacWindowStop(int dacChannel, int sample)
{
    lock_guard<mutex> lockOk(okMutex);

    if (dacChannel < 0 || dacChannel > 7) {
        cerr << "Error in Rhs2000EvalBoard::setDacWindowStop: dacChannel out of range." << endl;
        return;
    }

    if (sample < 0 || sample > 65535) {
        cerr << "Error in Rhs2000EvalBoard::setDacWindowStop: sample value out of range." << endl;
        return;
    }

    // Set sample for window to end on
...
void Rhs2000EvalBoard::changeThresholdType(int dacChannel, int typeIndex)
{
    lock_guard<mutex> lockOk(okMutex);

    if (dacChannel < 0 || dacChannel > 7) {
        cerr << "Error in Rhs2000EvalBoard::changeThresholdType: dacChannel out of range." << endl;
        return;
    }

    if (typeIndex < 0 || typeIndex > 1) {
        cerr << "Error in Rhs2000EvalBoard::changeThresholdType: typeIndex out of range." << endl;
    }
...
void Rhs2000EvalBoard::setDetectMode(bool enable)
{
    lock_guard<mutex> lockOk(okMutex);

    detectMode = enable; // Update software variable as well (for UI changes)
...
bool Rhs2000EvalBoard::getDetectMode()
{
    if (detectMode) {
        return true;
    } else {
...
void Rhs2000EvalBoard::setTtlOutMode(QVector<bool> enable)
{
    lock_guard<mutex> lockOk(okMutex);
...
bool Rhs2000EvalBoard::getTtlOutMode(int channel)
{
    if (channel < 0 || channel > 7) {
        cerr << "Error in Rhs2000EvalBoard::getTtlOutMode: channel out of range." << endl;
        return(false);
    }
...
// useFastSettle true = amplifier fast settle (legacy mode from Rhs2000 series chips)
