...
th_2_to_tb=round(th_2/0.195)+ 32768; %uint16
...
board_dac_data_u16=32768+round(board_dac_data(2,range_to_plot)./312.5e-6 ); %back to uint16
...
plot(h(1).XLim,[th_2_to_tb th_2_to_tb],'r')
...
retrieved_bin_matrix_dec_DAC_V = 312.5e-6 * (tb_DAC_uint16 - 32768);   % units = V a
retrieved_bin_matrix_dec_DAC_uV = retrieved_bin_matrix_dec_DAC_V*1e6;  % units = mV
retrieved_bin_matrix_dec_DAC_uV_pre_gain=0.195 * (tb_DAC_uint16 - 32768); %extracting as ADC we're at the same point
% of the comparator. 

online_res_V=board_dac_data(1,range_to_plot); %% these are Volts!!
online_res_uV=online_res_V*1e6;
online_res_uV_pre_gain=1e-6*online_res_uV/uV_per_uV;

figure

h(1)=subplot(5,1,1);
plot(time_s,retrieved_bin_matrix_dec_DAC_uV_pre_gain)
hold on
...
h(2)=subplot(5,1,2);
plot(time_s,online_res_uV_pre_gain)
hold on
plot(h(1).XLim,[th_1 th_1],'g')
plot(h(1).XLim,[th_2 th_2],'r')
