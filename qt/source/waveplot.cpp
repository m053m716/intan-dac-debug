...
void WavePlot::setSelectedDACChannelIndex(int index)
{
    if (index < 0 || index > 7) {
        cerr << "Error in WavePlot::setSelectedDACChannelIndex: index out of range." << endl;
    } else {
        selectedDacChannelIndex = index;
        emit selectedDACChannelStreamChanged(selectedDacChannel());
    }
}
