function x = readModifiedIntan(filename)
%% READMODIFIEDINTAN    Read modified contents with different metadata in DAC streams
%
% Version 1.1, 21 January 2019
%
...
new_dac_channel = struct(dac_trigger_struct);
spike_triggers = struct(spike_trigger_struct);
dac_triggers = struct(dac_trigger_struct);
dig_in_triggers = struct(spike_trigger_struct);
dig_out_triggers = struct(spike_trigger_struct);
...
for signal_group = 1:number_of_signal_groups
   ...
   if (signal_group_num_channels > 0 && signal_group_enabled > 0)
      ...
      for signal_channel = 1:signal_group_num_channels
         ...
         if signal_type == 4 % DAC
            fread(fid, 1, 'int16'); % nothing in "voltage_trigger_mode" stream
%             new_dac_channel(1).('open') = fread(fid, 1, 'int16');
            new_dac_channel(1).voltage_threshold = fread(fid, 1, 'int16');
            new_dac_channel(1).amp_trigger_channel = fread(fid, 1, 'int16');
            new_dac_channel(1).trigger_window_type = fread(fid, 1, 'int16');
         ...
         if (channel_enabled)
            switch (signal_type)
               ...
               case 4
                  ...
                  dac_triggers(board_dac_index) = new_dac_channel;
                  board_dac_index = board_dac_index + 1;
               case 5
                  board_dig_in_channels(board_dig_in_index) = new_channel;
                  dig_in_triggers(board_dig_in_index) = new_trigger_channel;
                  board_dig_in_index = board_dig_in_index + 1;
               case 6
                  board_dig_out_channels(board_dig_out_index) = new_channel;
                  dig_out_triggers(board_dig_out_index) = new_trigger_channel;
...
x = struct;
x.notes = notes;
x.frequency_parameters = frequency_parameters;
x.stim_parameters = stim_parameters;
if (data_file_main_version_number > 1)
   x.reference_channel = reference_channel;
end

if (num_amplifier_channels > 0)
   x.amplifier_channels = amplifier_channels;
   if (data_present)
      x.amplifier_data = amplifier_data;
      if (dc_amp_data_saved ~= 0)
         x.dc_amplifier_data = dc_amplifier_data;
      end
      x.stim_data = stim_data;
      x.amp_settle_data = amp_settle_data;
      x.charge_recovery_data = charge_recovery_data;
      x.compliance_limit_data = compliance_limit_data;
      x.t = t;
   end
   x.spike_triggers = spike_triggers;
end
if (num_board_adc_channels > 0)
   x.board_adc_channels = board_adc_channels;
   if (data_present)
      x.board_adc_data = board_adc_data;
   end
end
if (num_board_dac_channels > 0)
   x.board_dac_channels = board_dac_channels;
   if (data_present)
      x.board_dac_data = board_dac_data;
   end
   x.dac_triggers = dac_triggers;
end

if (num_board_dig_in_channels > 0)
   x.board_dig_in_channels = board_dig_in_channels;
   if (data_present)
      x.board_dig_in_data = board_dig_in_data;
   end
   x.dig_in_triggers = dig_in_triggers;
end
if (num_board_dig_out_channels > 0)
   x.board_dig_out_channels = board_dig_out_channels;
   if (data_present)
      x.board_dig_out_data = board_dig_out_data;
   end
   x.dig_out_triggers = dig_out_triggers;
end
